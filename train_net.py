...
class Trainer(DefaultTrainer):
    """
    Extension of the Trainer class adapted to SCORE.
    """
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        if evaluator_type == "coco":
            evaluator_list.append(COCOEvaluator(dataset_name, max_dets_per_image=300, tasks=['segm'], output_dir=output_folder))
...
def setup(args):
    ...
    add_score_config(cfg)
    cfg.merge_from_file(args.config_file)
    cfg.merge_from_list(args.opts)
    cfg.freeze()
    default_setup(cfg, args)
    # Setup logger for "score" module
    setup_logger(output=cfg.OUTPUT_DIR, distributed_rank=comm.get_rank(), name="score")
    return cfg
