...
from score.modeling.pixel_decoder.ops.modules import MSDeformAttn
import math
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, dense=False, mask=None):
        ...
        if (x.shape[1] != self.positional_embedding.shape[0]):
            ...
            #TODO
            x = x + self.resized_pos_embed(self.input_resolution, (h, w)).to(x.dtype)
        else:
        ...
        x, hidden_states = self.transformer(x, dense, mask)
        ...
        if x.dim() == 3:
            ...
            #TODO
            x = x[:, 1:, :].reshape(b, h, w, -1)
            for i in range(len(hidden_states)):
                hidden_states[i] = hidden_states[i].reshape(b, h, w, -1)

        return cls, x, hidden_states
    ...
    def resized_pos_embed(self, in_res, tgt_res, mode="bicubic"):
        ...
        #TODO
        resized_pos_embed = F.interpolate(pos_embed, size=(tgt_h, tgt_w), mode=mode, align_corners=False,) # 1 D S S -> 1 D S' S'
