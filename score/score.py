...
from .modeling.backbone.clip_rs import CLIP_RS
from .modeling.transformer_decoder.score_transformer_decoder import MaskPooling, get_classification_logits
import numpy as np
...
class SCORE(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.test_text_classifier_rs = None
        ...
        self.gca = CA(input_dim=768, num=1)
      
    def prepare_class_names_from_metadata(self, metadata, train_metadata):
        def split_labels(x):
            res = []
            for x_ in x:
                x_ = x_.replace('_', ' ').lower()
                x_ = x_.replace('-', ' ').lower()
    ...
    def from_config(cls, cfg):
        ...
        return {
            ...
            "geometric_ensemble_alpha": cfg.MODEL.SCORE.GEOMETRIC_ENSEMBLE_ALPHA,
            "geometric_ensemble_beta": cfg.MODEL.SCORE.GEOMETRIC_ENSEMBLE_BETA,
            "ensemble_on_valid_mask": cfg.MODEL.SCORE.ENSEMBLE_ON_VALID_MASK
        }
    ...
    def forward(self, batched_inputs):
        ...
        clip_input_img = F.interpolate(images.tensor, size = [224, 224], mode='bilinear', align_corners=False) #[1, 3, 224, 224]
        clip = self.clip_rs(clip_input_img)

        features['rs_clip_vis_dense'] = clip['clip_vis_dense']

        C, dim = text_classifier.shape #[15, 768]
        clip_vis = clip['clip_global'] / clip['clip_global'].norm(dim=-1, keepdim=True) #[2, 768]

        bs, _ = clip_vis.shape
        text_classifier_new = text_classifier.expand(bs, -1, -1) #[2, 15, 768]
        text_classifier_new = text_classifier_new.flatten(0, 1).unsqueeze(1) #[30, 1, 768]
        clip_vis = torch.repeat_interleave(clip_vis, C, dim=0)
        clip_vis = clip_vis.unsqueeze(1) #[30, 1, 768]
        text_classifier_new = self.gca(text_classifier_new.float(), clip_vis.float()) #[30, 1, 768]
        text_classifier_new = text_classifier_new.reshape(bs, C, -1) #[2, 15, 768]

        text_classifier = torch.cat([text_classifier, F.normalize(self.void_embedding.weight, dim=-1)], dim=0)
        text_classifier_new = torch.cat([text_classifier_new, F.normalize(self.void_embedding.weight, dim=-1).expand(bs, -1, -1)], dim=1) #[2, 12, 768]
        features['text_classifier'] = text_classifier_new
        features['num_templates'] = num_templates
        outputs = self.sem_seg_head(features, is_train=self.training)
        ...
        if self.training:
            ...
            mask_for_pooling = F.interpolate(mask_pred_results, size=clip_feature.shape[-2:],
                                                mode='bilinear', align_corners=False) #[1, 300, 200, 200]->[1, 300, 7, 7]
