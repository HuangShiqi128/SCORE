...
from score.third_party import clip

@BACKBONE_REGISTRY.register()
class CLIP_RS(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        model_name = cfg.MODEL.SCORE.RSCLIP_MODEL_NAME
        pretrained= cfg.MODEL.SCORE.RSCLIP_PRETRAINED_WEIGHTS
        # download on local rank 0 first
        if comm.get_local_rank() == 0:
            create_model_and_transforms(model_name)
        comm.synchronize()
        ...
        self.clip_model, clip_preprocess = clip.load(model_name, device=device, jit=False)
        ...
        if 'convnext_' in model_name:
            ...
            if '-b' in model_name:
                ...
                ckpt_path = os.path.join(os.getcwd(), "RemoteCLIP", "RemoteCLIP-ViT-B-32.pt")
                ckpt = torch.load(ckpt_path, map_location="cpu")
            elif '-l' in model_name:
                self.output_channels = [1024, 1024, 1024, 1024, 1024]
                ckpt_path = os.path.join(os.getcwd(), "RemoteCLIP", "RemoteCLIP-ViT-L-14.pt")
                ckpt = torch.load(ckpt_path, map_location="cpu")
    ...
    def encode_text(self, text, prompt=None):
        if prompt is not None:
            ...
            x = self.clip_model.token_embedding(text).to(self.dtype) 
    ...
    def extract_features_vit(self, x):
        out = {}
        cls, dense, hidden  = self.clip_model.encode_image(x, dense=True)
        out['clip_global'] = cls
        out['clip_vis_dense'] = dense.permute(0, 3, 1, 2) #[2, 768, 16, 16]
        out['hidden_features'] = hidden
        return out
